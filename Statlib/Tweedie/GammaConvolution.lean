...
lemma real_betaIntegral_eq (a b : ℝ) (ha : 0 < a) (hb : 0 < b) :
  ...
  · have h_beta : (∫ t in (0 : ℝ)..1, t^(a - 1) * (1 - t)^(b - 1)) = Complex.betaIntegral (a : ℂ) (b : ℂ) := by
    ...
    rw [ ← Complex.Gamma_ofReal, ← Complex.Gamma_ofReal, ← Complex.Gamma_ofReal ] ; norm_cast at * ; aesop;
  · positivity
...
lemma integral_rpow_mul_rpow_sub (a b : ℝ) (ha : 0 < a) (hb : 0 < b) {z : ℝ} (hz : 0 < z) :
  ...
  have h_simplify : ∫ t in (0:ℝ)..1, (z * t) ^ (a - 1) * (z - z * t) ^ (b - 1) * z = z^(a - 1) * z^(b - 1) * z * ∫ t in (0:ℝ)..1, t ^ (a - 1) * (1 - t) ^ (b - 1) := by
...
lemma gamma_density_conv (a b r : ℝ) (ha : 0 < a) (hb : 0 < b) (hr : 0 < r) {z : ℝ} (hz : 0 < z) :
  ...
  convert congr_arg ( fun x : ℝ => ( r ^ a / Real.Gamma a ) * ( r ^ b / Real.Gamma b ) * Real.exp ( - ( r * z ) ) * x ) ( integral_rpow_mul_rpow_sub a b ha hb hz ) using 1;
  · rw [ intervalIntegral.integral_of_le hz.le, MeasureTheory.integral_Ioc_eq_integral_Ioo ];
    rw [ ← MeasureTheory.integral_const_mul ] ; refine' MeasureTheory.setIntegral_congr_fun measurableSet_Ioo fun x hx => _ ; rw [ gammaPDFReal, gammaPDFReal ]
    rw [ if_pos hx.1.le, if_pos ( by linarith [ hx.2 ] ) ] ; rw [ show - ( r * z ) = - ( r * x ) + ( r * x - r * z ) by ring ] ; rw [ Real.exp_add ]
    ring_nf
  · unfold gammaPDFReal; rw [ if_pos hz.le ]
    ring_nf
    rw [ Real.rpow_add hr ] ; norm_num [ ne_of_gt ( Real.Gamma_pos_of_pos ha ), ne_of_gt ( Real.Gamma_pos_of_pos hb ) ] ; ring
...
lemma gammaPDF_conv_inner (a b r : ℝ) (ha : 0 < a) (hb : 0 < b) (hr : 0 < r) {z : ℝ} (hz : 0 < z) :
  ...
  have h_conv : ∫ x in Set.Ioo 0 z, (r ^ a / Real.Gamma a * x ^ (a - 1) * Real.exp (-(r * x))) * (r ^ b / Real.Gamma b * (z - x) ^ (b - 1) * Real.exp (-(r * (z - x)))) = r ^ (a + b) / Real.Gamma (a + b) * z ^ (a + b - 1) * Real.exp (-(r * z)) := by
    convert congr_arg ( fun x : ℝ => x * ( r ^ a / Real.Gamma a ) * ( r ^ b / Real.Gamma b ) * Real.exp ( - ( r * z ) ) ) ( integral_rpow_mul_rpow_sub a b ha hb hz ) using 1
      <;> ring_nf
    · rw [ ← MeasureTheory.integral_Ioc_eq_integral_Ioo, ← intervalIntegral.integral_of_le hz.le ] ; norm_num [ mul_assoc, mul_comm, mul_left_comm, ← Real.exp_add ] ; ring_nf
      exact Or.inl <| Or.inl <| by rw [ ← intervalIntegral.integral_const_mul ] ; congr ; ext ; ring_nf
    · rw [ Real.rpow_add hr ]
      ring_nf
      norm_num [ mul_assoc, mul_comm, mul_left_comm, ne_of_gt ( Real.Gamma_pos_of_pos ha ), ne_of_gt ( Real.Gamma_pos_of_pos hb ) ];
  ...
  · rw [ ← MeasureTheory.lintegral_indicator ] <;> norm_num [ Set.indicator ];
    ...
    filter_upwards [ MeasureTheory.measure_eq_zero_iff_ae_notMem.1 ( MeasureTheory.measure_singleton 0 ), MeasureTheory.measure_eq_zero_iff_ae_notMem.1 ( MeasureTheory.measure_singleton z ) ] with x hx₁ hx₂;
    split_ifs <;> simp_all
    · rw [ ← ENNReal.ofReal_mul ( by exact mul_nonneg ( mul_nonneg ( div_nonneg ( Real.rpow_nonneg hr.le _ ) ( Real.Gamma_nonneg_of_nonneg ha.le ) ) ( Real.rpow_nonneg ( by linarith ) _ ) ) ( Real.exp_nonneg _ ) ) ];
    · linarith;
    · linarith;
    · grind;
    · grind;
  · exact ( by contrapose! h_conv; rw [ MeasureTheory.integral_undef h_conv ] ; positivity );
  · filter_upwards [ MeasureTheory.ae_restrict_mem measurableSet_Ioo ] with x hx using mul_nonneg ( mul_nonneg ( mul_nonneg ( by positivity ) ( Real.rpow_nonneg hx.1.le _ ) ) ( Real.exp_nonneg _ ) ) ( mul_nonneg ( mul_nonneg ( by positivity ) ( Real.rpow_nonneg ( sub_nonneg.2 hx.2.le ) _ ) ) ( Real.exp_nonneg _ ) )
...
theorem gammaMeasure_conv (a b r : ℝ) (ha : 0 < a) (hb : 0 < b) (hr : 0 < r) :
  ...
  have h_gamma_def : ∀ c r : ℝ, 0 < c → 0 < r → gammaMeasure c r = volume.withDensity (fun x => gammaPDF c r x) := by
    aesop;
  -- Apply the equality of measures to rewrite the goal in terms of densities.
  suffices h_conv : ∀ {g : ℝ → ENNReal}, Measurable g → ∫⁻ z, g z ∂(volume.withDensity (fun x => gammaPDF a r x) ∗ volume.withDensity (fun x => gammaPDF b r x)) = ∫⁻ z, g z * gammaPDF (a + b) r z ∂volume by
    ext s hs; specialize @h_conv ( Set.indicator s 1 ) ; simp_all +decide [ Set.indicator ] ;
  ...
  intro g hg
  have h_conv : ∫⁻ z, g z ∂(volume.withDensity (fun x => gammaPDF a r x) ∗ volume.withDensity (fun x => gammaPDF b r x)) = ∫⁻ x, ∫⁻ y, g (x + y) * gammaPDF b r y * gammaPDF a r x ∂volume ∂volume := by
    ...
    · rw [ MeasureTheory.lintegral_withDensity_eq_lintegral_mul ];
      · simp +decide [mul_comm, mul_left_comm];
        congr! 1;
        ext x; rw [ MeasureTheory.lintegral_withDensity_eq_lintegral_mul ] ; simp +decide [mul_comm]
        · rw [ ← MeasureTheory.lintegral_const_mul' ] ; congr ; ext ; ring;
  ...
  have h_fubini : ∫⁻ x, ∫⁻ y, g (x + y) * gammaPDF b r y * gammaPDF a r x ∂volume ∂volume = ∫⁻ y, ∫⁻ x, g y * gammaPDF b r (y - x) * gammaPDF a r x ∂volume ∂volume := by
    have h_fubini : ∀ x, ∫⁻ y, g (x + y) * gammaPDF b r y * gammaPDF a r x ∂volume = ∫⁻ y, g y * gammaPDF b r (y - x) * gammaPDF a r x ∂volume := by
    ...
    · refine' AEMeasurable.mul _ _;
      ...
      · refine' Measurable.aemeasurable _;
        ...
        fun_prop;
    · refine' Measurable.aemeasurable _;
      exact Measurable.ennreal_ofReal ( measurable_gammaPDFReal a r |> Measurable.comp <| measurable_fst );
  -- By the properties of the gamma function, we know that $\int_{0}^{y} \gamma(a, r, x) \gamma(b, r, y-x) \, dx = \gamma(a+b, r, y)$ for $y > 0$.
  have h_gamma_conv : ∀ y > 0, ∫⁻ x, gammaPDF a r x * gammaPDF b r (y - x) ∂volume = gammaPDF (a + b) r y := by
    intro y hy; exact (by
    convert gammaPDF_conv_inner a b r ha hb hr hy using 1);
  rw [ h_conv, h_fubini ];
  rw [ MeasureTheory.lintegral_congr_ae ];
  filter_upwards [ MeasureTheory.measure_eq_zero_iff_ae_notMem.mp ( MeasureTheory.measure_singleton 0 ) ] with y hy;
  by_cases hy_pos : 0 < y;
  · rw [ ← h_gamma_conv y hy_pos, ← MeasureTheory.lintegral_const_mul ] ; congr ; ext ; ring;
    exact Measurable.mul ( by exact Measurable.ennreal_ofReal ( by exact measurable_gammaPDFReal _ _ ) ) ( by exact Measurable.ennreal_ofReal ( by exact measurable_gammaPDFReal _ _ |> Measurable.comp <| measurable_const.sub measurable_id' ) );
  · rw [ MeasureTheory.lintegral_congr_ae, MeasureTheory.lintegral_zero ];
    · simp +decide [ gammaPDF_of_neg ( show y < 0 from lt_of_le_of_ne ( le_of_not_gt hy_pos ) hy ) ];
    · filter_upwards [ ] with x ; by_cases hx : 0 < x <;> simp_all +decide
...
lemma finPiSum_eq_convPow (G : Measure ℝ) [IsProbabilityMeasure G] (n : ℕ) :
  ...
  · have h_eq : (Measure.pi (fun _ : Fin (n + 1) => G)).map (fun v => ∑ i : Fin (n + 1), v i) = (G.prod (Measure.pi (fun _ : Fin n => G))).map (fun p : ℝ × (Fin n → ℝ) => p.1 + ∑ i : Fin n, p.2 i) := by
      rw [ ← MeasureTheory.measurePreserving_piFinSuccAbove ( fun _ : Fin ( n + 1 ) => G ) 0 |>.map_eq ];
      rw [ MeasureTheory.Measure.map_map ];
      · congr with v ; simp +decide [ Fin.sum_univ_succ ];
        rfl;
      · fun_prop;
      · fun_prop;
...
lemma infinitePi_range_sum_eq_convPow (G : Measure ℝ) [IsProbabilityMeasure G] (n : ℕ) :
  ...
  have h_prod : (Measure.pi fun _ : Fin n => G) = Measure.map (fun y : ℕ → ℝ => fun i : Fin n => y i) (infinitePi fun _ => G) := by
  ...
  · congr! 1;
    ext; simp +decide [ Finset.sum_range ] ;
